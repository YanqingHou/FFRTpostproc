...
for filei=[141:160]%[GBDF(mediumm),GBDF(weakm)]%SatModel([mediumm])%BTF%(modelfiles)
    filename=opts(filei).filename;%fgetl(fidfs)
    filenametxt=strcat('../simuresult/','FFRTModel',num2str(filei),'.txt');
...
%% draw the plot of mu vs PfILS for only one pfreq
close all;
figure(1);
%     subplot(2,2,1);
...
%          subplot(2,2,2);
%     subplot(2,3,imodel+3);
scatter(1-PfPfix{ifig,1}(:,ffrtclnPsILS),PfPfix{ifig,1}(:,ffrtclnPftrue),5,PfPfix{ifig,1}(:,ffrtclnns),'filled'); colorbar; %hold on;
xlim([0,1]);
ylim([0,1]);
%     caxis([cmin,cmax]);
%
xlabel('$P_{f,ILS}$','interpreter','Latex','FontSize',fontsize);
ylabel('$P_{f}$','interpreter','Latex','FontSize',fontsize);
set(gca,'FontSize',fontsize,'yscale','log','xscale','log');
...
figure(4);
scatter(1-PfPfix{ifig,1}(:,ffrtclnPsILS),1-PfPfix{ifig,1}(:,ffrtclnPscon),5,PfPfix{ifig,1}(:,ffrtclnns),'filled'); colorbar; %hold on;
xlim([0,1]);
ylim([0,1]);
xlabel('$P_{f,ILS}$','interpreter','Latex','FontSize',fontsize);
ylabel('$P_{ff}$','interpreter','Latex','FontSize',fontsize);
set(gca,'FontSize',fontsize,'yscale','log','xscale','log');
set(gca,'FontSize',fontsize,'yscale','log');
...
figure(5);
scatter(1-PfPfix{ifig,1}(:,ffrtclnPsILS),PfPfix{ifig,1}(:,ffrtclnPfalsealarm),5,PfPfix{ifig,1}(:,ffrtclnns),'filled'); colorbar; %hold on;
xlim([0,1]);
ylim([0,1]);
xlabel('$P_{f,ILS}$','interpreter','Latex','FontSize',fontsize);
ylabel('$P_{fa}$','interpreter','Latex','FontSize',fontsize);
set(gca,'FontSize',fontsize,'yscale','log','xscale','log');
set(gca,'FontSize',fontsize,'yscale','log');
%     set(gcf,'units','normalized','outerposition',[0 0 1 1]);
colormap hsv;%jet
%% group the data for curve fitting
myfitgroup=cell(maxns,Pfslen);
for ifig=2%1:Pfslen%15%[1,6,10,15]%
    %     figure(ifig);
    %     for imodel=1:3
    %     subplot(2,3,imodel);
    %     subplot(2,1,1);
