function [fitresult, gof,FA,FT,FK,FPV] = FitmuwithFtestregress(Pf, mu,ns,alpha,drawswitch)
%CREATEFITS(PF,MU)
%  Create fits.
%
%  Data for 'ns1_power1' fit:
%      X Input : Pf
%      Y Output: mu
%  Data for 'ns1_power2' fit:
%      X Input : Pf
%      Y Output: mu
%  Data for 'exp2' fit:
%      X Input : Pf
%      Y Output: mu
%  Data for 'ns1_rat21' fit:
%      X Input : Pf
%      Y Output: mu
%  Output:
%      fitresult : a cell-array of fit objects representing the fits.
%      gof : structure array with goodness-of fit info.
%
%  See also FIT, CFIT, SFIT.

%  Auto-generated by MATLAB on 21-Oct-2015 14:24:05

%% Initialization.
% i=2;Pf=myfitgroup{i,1}(:,1);mu=myfitgroup{i,1}(:,2);Pfix=myfitgroup{i,1}(:,3);
% ns=i;
% Initialize arrays to store fits and goodness-of-fit.
fontsize=15;
fitresult = cell( 4, 1 );
gof = struct( 'sse', cell( 4, 1 ), ...
    'rsquare', [], 'dfe', [], 'adjrsquare', [], 'rmse', [] );
gof_orig=gof;
maxmu=max(mu);
minmu=min(mu);
txtwidth=(maxmu-minmu)/8;
txtposy=maxmu-txtwidth;

minPf=min(Pf);
maxPf=max(Pf);
txtposx=(minPf+maxPf)/6;
% iternum0=200;
% stopeps=0.001;
if ns<12
    ftpow = fittype( 'power1' );
else
    ftpow = fittype( 'power2' );
end
optspow1 = fitoptions( 'Method', 'NonlinearLeastSquares' );
optspow1.Algorithm = 'Levenberg-Marquardt';
optspow1.Display = 'Off';

FT=zeros(4,1); FK=FT; FA=FT;FPV=FT;
%% Fit: 'ns1_power1'.
[xData, yData] = prepareCurveData( Pf, mu );

% Set up fittype and options.
ft = fittype( 'power1' );
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Algorithm = 'Levenberg-Marquardt';
opts.Display = 'Off';

% opts.StartPoint = [0.0106945315846686 -0.975627188324542];
figure( 'Name', strcat('ns=',num2str(ns)));

% Fit model to data.

[fitresult{1}, gof(1)] = fit( xData, yData, ft, opts );
gof_orig(1)=gof(1);
% gof = struct( 'sse', cell( 4, 1 ), ...
%     'rsquare', [], 'dfe', [], 'adjrsquare', [], 'rmse', [] );
DFE=gof_orig(1).dfe;%length(yData)-2;
DFM=2;
DFT=DFE+DFM;%length(yData)-1;
rsquare=gof_orig(1).rsquare;
FT(1)=rsquare/(1-rsquare)*DFE/DFM;
FK(1)=finv(1-alpha,DFM,DFE);
FPV(1)=1-fcdf(FT(1),DFM,DFE);
ci=predint(fitresult{1},xData);
[fitresult{1},gof(1)]=fit(xData,ci(:,1),ft,opts);
% Plot fit with data.

if drawswitch
subplot(2,2,1);
h = plot( fitresult{1}, xData, yData);%,'predobs', 0.9 );
% RealRMSE=0;
% funcname=formula(fitresult{1});
% paranames=coeffnames(fitresult{1});
paravalues=coeffvalues(fitresult{1});
parabnds=confint(fitresult{1});
% legend( h, '\mu_m_i_n vs. P_f_,_I_L_S', 'y=a*x^b','Location', 'SouthWest' );
legend( h, {'$\mu_{min}$', '$\hat{\mu}_{min}$'},'interpreter','Latex','FontSize',fontsize,'Location', 'NE' );

title('Power1: y=a*x^b');
xlabel ('P_f_,_I_L_S');
ylabel ('\mu_m_i_n');

str1=strcat('a= ',num2str(paravalues(1)),', 95% [',num2str(parabnds(1,1)),',',num2str(parabnds(2,1)),' ]');
str2=strcat('b= ',num2str(paravalues(2)),', 95% [',num2str(parabnds(1,2)),',',num2str(parabnds(2,2)),' ]');
str3=strcat('SSE: ',num2str(gof(1).sse));
str4=strcat('ad R^2: ',num2str(gof(1).adjrsquare));
str5=strcat('RMSE: ',num2str(gof(1).rmse));
% str6=strcat('ReRMSE: ',num2str(RealRMSE));

% text(txtposx,txtposy,str1);
% text(txtposx,txtposy-txtwidth,str2);
% text(txtposx,txtposy-2*txtwidth,str3);
% text(txtposx,txtposy-3*txtwidth,str4);
text(txtposx,txtposy-0*txtwidth,str5);
% text(txtposx,txtposy-5*txtwidth,str6);
% set(gca,'FontSize',fontsize);

grid on
end
%% Fit: 'ns1_power2'.
[xData, yData] = prepareCurveData( Pf, mu );

% Set up fittype and options.
ft = fittype( 'power2' );
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Algorithm = 'Levenberg-Marquardt';

opts.Display = 'Off';
% opts.StartPoint = [0.0106945315846686 -0.975627188324542 0.030171985325082];

% Fit model to data.
% [fitresult{2}, gof(2)] = fit( xData, yData, ft, opts );

[fitresult{2}, gof(2)] = fit( xData, yData, ft, opts );
gof_orig(2)=gof(2);
DFE=gof_orig(2).dfe;%length(yData)-2;
DFM=3;
DFT=DFE+DFM;%length(yData)-1;
rsquare=gof_orig(2).rsquare;
FT(2)=rsquare/(1-rsquare)*DFE/DFM;
FK(2)=finv(1-alpha,DFM,DFE);
FPV(2)=1-fcdf(FT(2),DFM,DFE);

ci=predint(fitresult{2},xData);
[fitresult{2},gof(2)]=fit(xData,ci(:,1),ft,opts);

if drawswitch
subplot(2,2,2);
% Plot fit with data.
% figure( 'Name', 'ns1_power2' );
h = plot( fitresult{2}, xData, yData);%,'predobs', 0.9 );
% paranames=coeffnames(fitresult{2});
paravalues=coeffvalues(fitresult{2});
parabnds=confint(fitresult{2});
% legend( h, '\mu_m_i_n vs. P_f_,_I_L_S', 'y=a*x^b+c', 'Location', 'SouthWest' );
legend( h, {'$\mu_{min}$', '$\hat{\mu}_{min}$'},'interpreter','Latex','FontSize',fontsize,'Location', 'NE' );

title('Power2: y=a*x^b+c');
xlabel ('P_f_,_I_L_S');
ylabel ('\mu_m_i_n');

str1=strcat('a= ',num2str(paravalues(1)),', 95% [',num2str(parabnds(1,1)),',',num2str(parabnds(2,1)),' ]');
str2=strcat('b= ',num2str(paravalues(2)),', 95% [',num2str(parabnds(1,2)),',',num2str(parabnds(2,2)),' ]');
str3=strcat('c= ',num2str(paravalues(3)),', 95% [',num2str(parabnds(1,3)),',',num2str(parabnds(2,3)),' ]');

str4=strcat('SSE: ',num2str(gof(2).sse));
str5=strcat('ad R^2: ',num2str(gof(2).adjrsquare));
str6=strcat('RMSE: ',num2str(gof(2).rmse));
% str7=strcat('ReRMSE: ',num2str(RealRMSE));

% text(txtposx,txtposy,str1);
% text(txtposx,txtposy-txtwidth,str2);
% text(txtposx,txtposy-2*txtwidth,str3);
% text(txtposx,txtposy-3*txtwidth,str4);
% text(txtposx,txtposy-4*txtwidth,str5);
text(txtposx,txtposy-0*txtwidth,str6);
% text(txtposx,txtposy-6*txtwidth,str7);

% set(gca,'FontSize',fontsize);
grid on
end
%% Fit: 'exp2'.
[xData, yData] = prepareCurveData( Pf, mu );

% Set up fittype and options.
ft = fittype( 'exp2' );
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Algorithm = 'Levenberg-Marquardt';

opts.Display = 'Off';
% opts.StartPoint = [0.523479696751086 -35.5758773193383 0.668518289836885 -27.0231187478561];

% Fit model to data.
[fitresult{3}, gof(3)] = fit( xData, yData, ft, opts );
gof_orig(3)=gof(3);
DFE=gof_orig(3).dfe;%length(yData)-2;
DFM=4;
DFT=DFE+DFM;%length(yData)-1;
rsquare=gof_orig(3).rsquare;
FT(3)=rsquare/(1-rsquare)*DFE/DFM;
FK(3)=finv(1-alpha,DFM,DFE);
FPV(3)=1-fcdf(FT(3),DFM,DFE);

ci=predint(fitresult{3},xData);
if sum(isnan(ci))==0
    [fitresult{3},gof(3)]=fit(xData,ci(:,1),ft,opts);
end
if drawswitch
subplot(2,2,3);
% Plot fit with data.
% figure( 'Name', 'exp2' );
h = plot( fitresult{3}, xData, yData);%,'predobs', 0.9 );
% paranames=coeffnames(fitresult{3});
paravalues=coeffvalues(fitresult{3});
parabnds=confint(fitresult{3});
% legend( h, '\mu_m_i_n vs. P_f_,_I_L_S', 'y= a*exp(b*x) + c*exp(d*x)','Location', 'SouthWest' );
legend( h, {'$\mu_{min}$', '$\hat{\mu}_{min}$'},'interpreter','Latex','FontSize',fontsize,'Location', 'NE' );

title('exp2:y= a*exp(b*x) + c*exp(d*x)');
xlabel ('P_f_,_I_L_S');
ylabel ('\mu_m_i_n');

% str1=strcat('a= ',num2str(paravalues(1)),', 95% [',num2str(parabnds(1,1)),',',num2str(parabnds(2,1)),' ]');
% str2=strcat('b= ',num2str(paravalues(2)),', 95% [',num2str(parabnds(1,2)),',',num2str(parabnds(2,2)),' ]');
% str3=strcat('c= ',num2str(paravalues(3)),', 95% [',num2str(parabnds(1,3)),',',num2str(parabnds(2,3)),' ]');
% str4=strcat('d= ',num2str(paravalues(4)),', 95% [',num2str(parabnds(1,4)),',',num2str(parabnds(2,4)),' ]');
% 
% str5=strcat('SSE: ',num2str(gof(3).sse));
% str6=strcat('ad R^2: ',num2str(gof(3).adjrsquare));
str7=strcat('RMSE: ',num2str(gof(3).rmse));
% str8=strcat('ReRMSE: ',num2str(RealRMSE));

% text(txtposx,txtposy,str1);
% text(txtposx,txtposy-txtwidth,str2);
% text(txtposx,txtposy-2*txtwidth,str3);
% text(txtposx,txtposy-3*txtwidth,str4);
% text(txtposx,txtposy-4*txtwidth,str5);
% text(txtposx,txtposy-5*txtwidth,str6);
text(txtposx,txtposy-0*txtwidth,str7);
% text(txtposx,txtposy-7*txtwidth,str8);

% set(gca,'FontSize',fontsize);

grid on
end
%% Fit: 'ns1_rat21'.
[xData, yData] = prepareCurveData( Pf, mu );

% Set up fittype and options.
ft = fittype( 'rat21' );
opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
opts.Algorithm = 'Levenberg-Marquardt';

opts.Display = 'Off';
% opts.StartPoint = [0.933993247757551 0.678735154857773 0.757740130578333 0.743132468124916];

% Fit model to data.
[fitresult{4}, gof(4)] = fit( xData, yData, ft, opts );
gof_orig(4)=gof(4);
DFE=gof_orig(4).dfe;%length(yData)-2;
DFM=4;
DFT=DFE+DFM;%length(yData)-1;
rsquare=gof_orig(4).rsquare;
FT(4)=rsquare/(1-rsquare)*DFE/DFM;
FK(4)=finv(1-alpha,DFM,DFE);
FPV(4)=1-fcdf(FT(4),DFM,DFE);

ci=predint(fitresult{4},xData);
if sum(isnan(ci))==0
    [fitresult{4},gof(4)]=fit(xData,ci(:,1),ft,opts);
end
if drawswitch
subplot(2,2,4);
% Plot fit with data.
% figure( 'Name', 'ns1_rat21' );
h = plot( fitresult{4}, xData, yData);%'predobs', 0.9 );
% paranames=coeffnames(fitresult{4});
paravalues=coeffvalues(fitresult{4});
parabnds=confint(fitresult{4});
% legend( h, '\mu_m_i_n vs. P_f_,_I_L_S', 'y= (p1*x^2 + p2*x + p3) / (x + q1)','Location', 'NE' );
legend( h, {'$\mu_{min}$', '$\hat{\mu}_{min}$'},'interpreter','Latex','FontSize',fontsize,'Location', 'NE' );
%     ylabel('$\mu_{min}$','interpreter','Latex','FontSize',fontsize);

title('rat21:y= (p1*x^2 + p2*x + p3) / (x + q1)');
xlabel ('P_f_,_I_L_S');
ylabel ('\mu_m_i_n');

% str1=strcat('p1= ',num2str(paravalues(1)),', 95% [',num2str(parabnds(1,1)),',',num2str(parabnds(2,1)),' ]');
% str2=strcat('p2= ',num2str(paravalues(2)),', 95% [',num2str(parabnds(1,2)),',',num2str(parabnds(2,2)),' ]');
% str3=strcat('p3= ',num2str(paravalues(3)),', 95% [',num2str(parabnds(1,3)),',',num2str(parabnds(2,3)),' ]');
% str4=strcat('q1= ',num2str(paravalues(4)),', 95% [',num2str(parabnds(1,4)),',',num2str(parabnds(2,4)),' ]');
% 
% str5=strcat('SSE: ',num2str(gof(4).sse));
% str6=strcat('ad R^2: ',num2str(gof(4).adjrsquare));
str7=strcat('RMSE: ',num2str(gof(4).rmse));
% str8=strcat('ReRMSE: ',num2str(RealRMSE));

% text(txtposx,txtposy,str1);
% text(txtposx,txtposy-txtwidth,str2);
% text(txtposx,txtposy-2*txtwidth,str3);
% text(txtposx,txtposy-3*txtwidth,str4);
% text(txtposx,txtposy-4*txtwidth,str5);
% text(txtposx,txtposy-5*txtwidth,str6);
text(txtposx,txtposy-0*txtwidth,str7);
% text(txtposx,txtposy-7*txtwidth,str8);

% set(gca,'FontSize',fontsize);
grid on

set(gcf,'units','normalized','outerposition',[0 0 1 1]);
end
if drawswitch
fontsize=15;
funcnames=cell(1,4);
funcnames{1}='$y=p_1x^{p_2}$';
funcnames{2}='$y=p_1x^{p_2}+p_3$';
funcnames{3}='$y=p_1e^{p_2x}+p_3e^{p_4x}$';
funcnames{4}='$y=(p_1x^2+p_2x+p_3)/(x+p_4)$';



for i=1:4
    subplot(2,2,i);
    xlabel('$P_{f,ILS}$','interpreter','Latex','FontSize',fontsize);
    ylabel('$\mu_{min}$','interpreter','Latex','FontSize',fontsize);
    title(funcnames{i},'interpreter','Latex','FontSize',fontsize);
    set(gca,'FontSize',fontsize);
end
end
%% Ftest
% alpha=0.1;
% T=gof_orig(2).rmse^2/gof_orig(4).rmse^2;
% Fdeg1=length(mu)-3; Fdeg2=length(mu)-4;
% K=finv(1-alpha,Fdeg1,Fdeg2);
% Accept=T<K;




% T=gof_orig(2).rmse^2/gof_orig(4).rmse^2;
% Fdeg1=1; Fdeg2=length(mu)-4;
% K=finv(1-alpha,Fdeg1,Fdeg2);
% Accept=T<K;

FA=FT<FK;