% function Pfhist(Fitfuncs)

load FitFuncsMuPfILS;
maxns=size(fitfuncs,1);% fitfuncs= cell( nsttl, Pfslen );
...
for filei=[GBDF(mediumm),GBDF(weakm)]%SatModel([mediumm])%BTF%(modelfiles)
    ...
    for ifig=1:Pfslen
        ...
        for ep=1:336
            eprows=resepa(:,ffrtclnep)==ep;
            resepb=resepa(eprows,:);
            for ns=1:maxns
                nsrows=resepb(:,ffrtclnns)==ns;
                
                fitf=fitfuncs{ns,ifig}; 
                muhat=fitf(Pfreq);
                if muhat>1, muhat=1; end
                flags=resepb(nsrows,ffrtclnmu)<=muhat;
                
                [indx]=find(resepb(nsrows,ffrtclnmu)<=muhat,'last');
                if isempty(indx) % means muhat is 0, which is nearly impossible. If it is 0, then the failure rate should be 0.
                    Pftrue=0;
                else
                    Pftrue=resepb(indx,ffrtclnPftrue);
                end
                pcnts(ifig,1)=pcnts(ifig,1)+1;
                pcnts(ifig,2)=pcnts(ifig,2)+Pftrue<=Pfreq;
...
clear resep;  clear resepb; clear nsrows; clear posiPfixrows; clear Pfreqrows;

pcnts(:,3)=pcnts(:,1)./pcnts(:,2);
figure
plot(pcnts(:,3));
% for i=1:Pfslenclear resepa;
%     for j=1:maxns
...
%          myfitgroup{ns,1}=[minPf_ILS,minmu,minPfix];
% % figure( 'Name', 'y=a*x^b' );
% %
%
% % % Plot fit with data.
